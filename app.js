...
async function getGh(path) { const c = ghCfg(); const r = await fetch(`https://api.github.com/repos/${c.repo}/contents/${path}?ref=${encodeURIComponent(c.branch)}`, { headers: ghHeaders(c.token) }); if (r.status === 404) return null; if (!r.ok) throw await ghFail(r); return r.json(); }
async function putGh(path, content, message, sha, is64) { const c = ghCfg(); const body = { message, branch: c.branch, content: is64 ? content : b64(content) }; if (sha) body.sha = sha; const r = await fetch(`https://api.github.com/repos/${c.repo}/contents/${path}`, { method: "PUT", headers: ghHeaders(c.token), body: JSON.stringify(body) }); if (!r.ok) throw await ghFail(r); return r.json(); }
async function ghFail(r) { const body = await r.text().catch(() => ""); if (r.status === 401) { localStorage.removeItem(GHTOKEN); sessionStorage.removeItem(GHTOKEN); return new Error(tr("tokenInvalid")); } return new Error(`${r.status} ${r.statusText} ${body}`.trim()); }
